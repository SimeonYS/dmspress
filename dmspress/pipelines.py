...
class DmspressPipeline:

    # Database setup
    conn = sqlite3.connect('dmspress.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `dmspress`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM dmspress WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO dmspress (date, title, link, content)"
